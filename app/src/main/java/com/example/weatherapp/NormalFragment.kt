...
class NormalFragment :Fragment(){
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.mainscreen, container,false)
        ...
        change.setOnClickListener(){
            val action = NormalFragmentDirections.actionNormalfragmentToElderfragment(place.text.toString())
        ...
        locate.setOnClickListener(){
            ...
            builder.setMessage("Wprowadź nazwę miasta")
    ...
    open fun getWeatherDetails(city:String) {
        ...
        if ((city == "")) {
            ...
            var stringRequest: StringRequest? = StringRequest(
                ...
                tempUrl,
                object : com.android.volley.Response.Listener<kotlin.String?> {
                    override open fun onResponse(response: String?) {
                        try {
                            ...
                            var sunrisedf: Date? = Date(sunrisedv)
                            var sunrisetime: String? = SimpleDateFormat("HH:mm").format(sunrisedf)

                            var sunset: String? = jsonObjectSys?.getString("sunset")
                            var sunsetdv: Long =
                                java.lang.Long.valueOf(sunset) * 1000

                            var sunsetdf: Date? = Date(sunsetdv)
                            var sunsettime: String? = SimpleDateFormat("HH:mm").format(sunsetdf)

                            var ctime: String? = jsonResponse?.getString("dt")
                            var timedv: Long =
                                java.lang.Long.valueOf(ctime) * 1000

                            var timedf: Date? = Date(timedv)
                            var currenttime: String? = SimpleDateFormat("dd MMM yyy | HH:mm").format(timedf)
                            var iconid: String? =
